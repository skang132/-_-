...
class MultiSignal(gym.Env):
    ...
    def reset(self):
        ...
        self.sumo_cmd += ['--random', '--time-to-teleport', '-1', '--tripinfo-output',
                            ...
                            '--vehroute-output', os.path.join(self.log_dir, self.connection_name, 'route_' + str(self.run) + '.xml'),
                            '--vehroute-output.sorted', 'True',
                            '--vehroute-output.write-unfinished', 'True',
                            '--collision-output',
                            os.path.join(self.log_dir, self.connection_name, 'coll_' + str(self.run) + '.xml'),
                            '--statistic-output',
                            os.path.join(self.log_dir, self.connection_name, 'stats_' + str(self.run) + '.xml'),
                            '--no-step-log', 'True',
                            '--no-warnings', 'True',]
        if self.libsumo:
